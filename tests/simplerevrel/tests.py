...
class RelatedTests(base.TestCase):
    ...
    def test_not_in_reverse_rel_option_fields(self):
        ...
        self.assertNotIn(
            self.models.Project._meta.private_fields[0],
    ...
    def test_get_gm2m_models(self):
        ...
        self.assertListEqual(
            self.models.Links.related_objects.get_related_models(include_auto=True),
...
class ReverseOperationsTest(base.TestCase):
    ...
    def test_reverse_add(self):
        ...
        self.assertListEqual(list(self.links2.related_objects.all()), [self.project])
...
class FilterTests(base.TestCase):
    ...
    def test_filter_by_model(self):
        ...
        self.assertSetEqual(
            set(
                self.links.related_objects.filter(
    ...
    def test_reverse_chain_filter(self):
        self.assertEqual(
            self.models.Project.objects.filter(links__name="Links")[0], self.project
        )
...
class PrefetchTests(base.TestCase):
    def setUp(self):
        ...
        projects = [self.models.Project.objects.create() for _ in range(5)]
        tasks = [
            [self.models.Task.objects.create() for _ in range(5)] for _ in range(5)
        ]
        links = [self.models.Links.objects.create() for _ in range(5)]
    ...
    def test_prefetch_forward(self):
        with self.assertNumQueries(4):
            ...
            prefetched = [
                set(l.related_objects.all())
                for l in self.models.Links.objects.prefetch_related("related_objects")
            ]
        ...
        with self.assertNumQueries(16):
            ...
            normal = [
                set(l.related_objects.all()) for l in self.models.Links.objects.all()
            ]
    ...
    def test_prefetch_reverse(self):
        with self.assertNumQueries(2):
            ...
            prefetched = [
                set(t.links_set.all())
                for t in self.models.Task.objects.prefetch_related("links_set")
            ]
        ...
        with self.assertNumQueries(26):
            ...
            normal = [set(t.links_set.all()) for t in self.models.Task.objects.all()]
