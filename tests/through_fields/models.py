...
class Links(models.Model):
    ...
    related_objects = gm2m.GM2MField(
        Project, through="RelLinks", through_fields=("links", "target")
    )
...
class RelLinks(models.Model):
    ...
    other_fk = models.ForeignKey(
        Links, null=True, related_name="other_rellinks", on_delete=models.CASCADE
    )
    links = models.ForeignKey(Links, on_delete=models.CASCADE)

    other_gfk = GenericForeignKey(ct_field="other_gfk_ct", fk_field="other_gfk_fk")
    other_gfk_ct = models.ForeignKey(
        ContentType, null=True, related_name="other_rellinks", on_delete=models.CASCADE
    )
