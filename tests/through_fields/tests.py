...
class ThroughFieldTests(base.TestCase):
    def setUp(self):
        ...
        self.rellink = self.models.RelLinks.objects.create(
            links=self.links, target=self.project1
        )

    def test_accessors(self):
        self.assertListEqual([self.links], list(self.project1.links_set.all()))
        self.assertListEqual([self.project1], list(self.links.related_objects.all()))
