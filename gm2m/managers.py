...
class GM2MBaseManager(Manager):
    ...
    def get_queryset(self):
        try:
            ...
            return (
                self._get_queryset(using=db)
                ._next_is_sticky()
                .filter(**self.core_filters)
            )

    def _get_queryset(self, using):
        return super().get_queryset().using(using)
    ...
    def get_prefetch_queryset(self, instances, queryset=None):
        ...
        qs, rel_obj_attr, instance_attr = self._get_prefetch_queryset_params(
            instances, queryset, db
        )

        return (qs, rel_obj_attr, instance_attr, False, self.prefetch_cache_name, False)
    ...
    def _get_extra_queryset(self, queryset, q, extra_fields, db):
        ...
        extra = dict(
            select=dict(
                (
                    f"_prefetch_related_val_{f.attname}",
                    f"{qn(join_table)}.{qn(f.column)}",
                )
    ...
    def _check_through_model(self, method_name):
        ...
        if not self.through._meta.auto_created:
            ...
            raise AttributeError(
                f"Cannot use {method_name}() on a ManyToManyField "
                f"which specifies an intermediary model. "
    ...
    def _do_clear(self, db, filter=None):
        self.through._default_manager.using(db).filter(**(filter or {})).delete()
...
class GM2MBaseSrcManager(Manager):
    def __init__(self, instance):
        ...
        self.core_filters[
            f"{self.query_field_name}__{self.field_names['tgt_ct']}"
        ] = get_content_type(self.instance)
        self.core_filters[
            f"{self.query_field_name}__{self.field_names['tgt_fk']}"
        ] = self.instance.pk
    ...
    def _get_prefetch_queryset_params(self, instances, queryset, db):
        ...
        for obj in instances:
            q = q | Q(
                **{
                    f"{self.query_field_name}__{self.field_names['tgt_ct']}": get_content_type(
                        obj
                    ),
                    f"{self.query_field_name}__{self.field_names['tgt_fk']}": obj.pk,
                }
            )
        ...
        def rel_obj_attr(relobj):
            ...
            for f in extra_fields:
                try:
                    ...
                    model = ct.ContentType.objects.get_for_id(t[0]).model_class()
                except IndexError:
                    # t is empty
                    model = ct.ContentType
                t.append(
                    model._meta.pk.to_python(
                        getattr(relobj, f"_prefetch_related_val_{f.attname}")
                    )
                )
            return tuple(t)

        # model attribute retrieval function
        instance_attr = lambda inst: (get_content_type(inst).pk, inst.pk)
        ...
        for obj in objs:
            if obj.pk not in vals:
                to_add.append(
                    self.through(
                        **{
                            f"{self.field_names['src']}_id": obj.pk,
                            self.field_names["tgt_ct"]: inst_ct,
                            self.field_names["tgt_fk"]: self.pk,
                        }
                    )
    ...
    def _to_remove(self, objs):
        ...
        return Q(
            **{
                f"{self.field_names['src']}_id__in": [obj.pk for obj in objs],
                self.field_names["tgt_ct"]: inst_ct,
                self.field_names["tgt_fk"]: self.pk,
            }
        )
...
class GM2MBaseTgtManager(Manager):
    def __init__(self, instance):
        ...
        source_field = self.through._meta.get_field(self.field_names["src"])
        self.source_related_fields = source_field.related_fields
        for __, rh_field in self.source_related_fields:
            key = f"{self.query_field_name}__{rh_field.name}"
            self.core_filters[key] = getattr(self.instance, rh_field.attname)
    ...
    def _get_prefetch_queryset_params(self, instances, queryset, db):
        ...
        for lh_field, rh_field in self.source_related_fields:
            query[f"{lh_field.name}__in"] = set(
                getattr(obj, rh_field.attname) for obj in instances
            )
        q = Q(**query)
        ...
        def rel_obj_attr(relobj):
            t = []
            for f in extra_fields:
                v = getattr(relobj, f"_prefetch_related_val_{f.attname}")
                try:
                    v = v.pop()
        ...
        instance_attr = lambda inst: tuple(
            [getattr(inst, f.attname) for f in select_fields]
        )
    ...
    def _to_add(self, objs, db):
        ...
        for obj in objs:
            ...
            objs_set.add((get_content_type(obj), obj.pk))
            m = obj.__class__
            if m not in models:
        ...
        vals = (
            ...
            .values_list(self.field_names["tgt_ct"], self.field_names["tgt_fk"])
        )
        ...
        for ct, pk in objs_set.difference(vals):
            to_add.append(
                self.through(
                    **{
                        f"{self.field_names['src']}_id": self.pk,
    ...
    def _to_remove(self, objs):
        ...
        return q & Q(**{f"{self.field_names['src']}_id": self.pk})

    def _to_clear(self):
        return {f"{self.field_names['src']}_id": self.pk}
    ...
    def _to_change(self, objs, db):
        ...
        for val in vals:
            # Convert the obj to (content_type, primary_key)
            rem_q = rem_q | Q(**{ct_fname: val[0], fk_fname: val[1]})

        return [
            self.through(**{f"{src_fname}_id": self.pk, ct_fname: t[0], fk_fname: t[1]})
            for t in to_add
        ], rem_q & Q(**{f"{src_fname}_id": self.pk})
