...
class GM2MRelation(ForeignObject):
    ...
    def __init__(self, model, field, rel, **kwargs):
        ...
        super(GM2MRelation, self).__init__(
            model, from_fields=[field.name], to_fields=[], **kwargs
        )
    ...
    def bulk_related_objects(self, objs, using=DEFAULT_DB_ALIAS):
        ...
        if on_delete is not DO_NOTHING:
            ...
            if on_delete in (DO_NOTHING_SIGNAL, CASCADE_SIGNAL, CASCADE_SIGNAL_VETO):
                results = deleting.send(sender=self.field, del_objs=objs, rel_objs=qs)
            else:
                results = []

            if (
                on_delete in (CASCADE, CASCADE_SIGNAL)
...
class GM2MUnitRel(ForeignObjectRel):
    ...
    def _check_referencing_to_swapped_model(self):
        if (
            self.model not in apps.get_models()
            and not isinstance(self.model, str)
            and self.model._meta.swapped
        ):
            model = f"{self.model._meta.app_label}.{self.model._meta.object_name}"
            return [
                checks.Error(
                    f"Field defines a relation with the model '{model}', "
                    f"which has been swapped out.",
    ...
    def _check_clashes(self):
        ...
        field_name = f"{opts.object_name}.{self.field.name}"  # i. e. "Model.gm2m"
        ...
        for clash_field in potential_clashes:
            clash_name = (
                f"{rel_opts.object_name}.{clash_field.name}"  # i. e. "Target.model_set"
            )
            if clash_field.name == rel_name:
                errors.append(
                    checks.Error(
                        f"Reverse accessor for '{field_name}' clashes "
                        f"with field name '{clash_name}'.",
                        hint=f"Rename field '{clash_name}', "
                        f"or add/change a related_name "
                        f"argument to the definition for "
                        f"field '{field_name}'.",
                        obj=self,
                        id="gm2m.E202",
                    )
            ...
            if clash_field.name == rel_query_name:
                errors.append(
                    checks.Error(
                        f"Reverse query name for '{field_name}' clashes "
                        f"with field name '{clash_name}'.",
                        hint=f"Rename field '{clash_name}', or "
                        f"add/change a related_name argument "
                        f"to the definition for field '{field_name}'.",
                        obj=self,
                        id="gm2m.E203",
                    )
                )
        ...
        potential_clashes = [
            r for r in rel_opts.related_objects if r.field is not self.field
        ]
        for clash_field in potential_clashes:
            # "Model.gm2m"
            clash_name = (
                f"{clash_field.model._meta.object_name}.{clash_field.field.name}"
            )
            if clash_field.get_accessor_name() == rel_name:
                errors.append(
                    checks.Error(
                        f"Reverse accessor for '{field_name}' clashes with "
                        f"reverse accessor for '{clash_name}'.",
            ...
            if clash_field.get_accessor_name() == rel_query_name:
                errors.append(
                    checks.Error(
                        f"Reverse query name for '{field_name}' clashes with"
                        f" reverse query name for '{clash_name}'.",
                        hint=f"Add or change a related_name argument to the definition"
                        f" for '{field_name}' or '{clash_name}'.",
                        obj=self,
                        id="gm2m.E205",
                    )
                )
    ...
    def contribute_to_class(self):
        if isinstance(self.model, str) or self.model._meta.pk is None:
            ...
            lazy_related_operation(
                resolve_related_class, self.field.model, self.model, rel=self
            )
        else:
    ...
    def contribute_to_related_class(self):
        ...
        if not self.is_hidden() and not self.field.model._meta.swapped:
            setattr(
                self.model,
                self.related_name or (self.field.model._meta.model_name + "_set"),
    ...
    def swappable_setting(self):
        ...
        if isinstance(self.model, str):
            ...
            to_string = f"{self.model._meta.app_label}.{self.model._meta.object_name}"
        # See if anything swapped/swappable matches
        for model in apps.get_models(include_swapped=True):
            if (
                model._meta.swapped == to_string
                or model._meta.swappable
                and (f"{model._meta.app_label}.{model._meta.object_name}") == to_string
            ):
    ...
    if django.VERSION >= (4, 0):
        ...
        def get_extra_restriction(self, alias, remote_alias):
            ...
            ct_pk = ct.ContentType.objects.get_for_model(
...
class GM2MRel(ManyToManyRel):
    ...
    def __init__(self, field, related_models, **params):
        ...
        for on_del_param in ("src", "tgt"):
            ...
            self.set_init(on_del_param, params.pop(on_del_param, self.on_delete))

        if self.through and not self.db_constraint:
            raise ValueError(
                "django-gm2m: Can't supply a through model " "with db_constraint=False"
            )
    ...
    def add_relation(self, model, on_delete=None, auto=False, contribute_to_class=True):
        try:
            assert (
                not model._meta.abstract
            ), f"{self.field.__class__.__name__} cannot define a relation with abstract class {model._meta.object_name}"
        except AttributeError:
            # model._meta doesn't exist, so it must be a string
            assert isinstance(model, str), (
                f"{self.field.__class__.__name__}({model!r}) is invalid. "
                f"First parameter to GM2MField must be either a model or a model name"
            )
    ...
    def _check_relationship_model(self, from_model=None, **kwargs):
        if hasattr(self.through, "_meta"):
            qualified_model_name = (
                f"{self.through._meta.app_label}.{self.through.__name__}"
            )
        else:
        ...
        if self.through not in apps.get_models(include_auto_created=True):
            ...
            errors.append(
                checks.Error(
                    f"Field specifies a many-to-many relation "
                    f"through model '{qualified_model_name}', which has not been installed.",
                    hint=None,
                    obj=self,
                    id="gm2m.E101",
                )
            ...
            if seen_from == 0:
                errors.append(
                    checks.Error(
                        f"The model is used as an intermediate model by '{self}', "
                        f"but it does not have a foreign key to '{from_model_name}' or a "
                        f"generic foreign key.",
                        hint=None,
                        obj=self.through,
                        id="gm2m.E102",
                    )
                ...
                errors.append(
                    checks.Warning(
                        f"The model is used as an intermediate model by '{self}', but it "
                        f"has more than one foreign key from '{from_model_name}', which is "
                        f"ambiguous. You must specify which foreign key Django should use "
                        f"via the through_fields keyword argument.",
                        hint=None,
                        obj=self,
                        id="gm2m.E103",
                    )
            ...
            if seen_to == 0:
                errors.append(
                    checks.Error(
                        f"The model is used as an intermediate model "
                        f"by '{from_model_name}', but it does not have a"
                        f" generic foreign key.",
                        hint=None,
                        obj=self.through,
                        id="gm2m.E104",
                ...
                errors.append(
                    checks.Warning(
                        f"The model is used as an intermediate model by '{self}', "
                        f"but it has more than one generic foreign key, which is ambiguous. "
                        f"You must specify which generic foreign key Django should use via "
                        f"the through_fields keyword argument.",
                        hint=None,
                        obj=self,
                        id="gm2m.E105",
                    )
        ...
        if self.through_fields is not None:
            ...
            if not (
                len(self.through_fields) >= 2
                and self.through_fields[0]
                and self.through_fields[1]
            ):
                errors.append(
                    checks.Error(
                        f"Field specifies 'through_fields' but does not "
                        f"provide the names of the two link fields that "
                        f"should be used for the relation through model '{qualified_model_name}'.",
            ...
            # expected models
                ...
                if possible_field_names:
                    hint = (
                        f"Did you mean one of the following foreign keys"
                        f" to '{from_model._meta.object_name}': {', '.join(possible_field_names)}?"
                    )
                else:
                    hint = None
                ...
                try:
                    ...
                    errors.append(
                        checks.Error(
                            f"The intermediary model '{qualified_model_name}' "
                            f"has no field '{src_field_name}'.",
                            hint=hint,
                            obj=self,
                            id="gm2m.E107",
                        )
                    ...
                    if not (
                        ...
                        and getattr(field.remote_field, "model", None) == from_model
                    ):
                        errors.append(
                            checks.Error(
                                f"'{through._meta.object_name}.{src_field_name}' is "
                                f"not a foreign key to '{from_model._meta.object_name}'.",
                                hint=hint,
                                obj=self,
                                id="gm2m.E108",
                            )
                ...
                if possible_field_names:
                    hint = (
                        f"Did you mean one of the following generic"
                        f" foreign keys: {', '.join(possible_field_names)}?"
                    )
                else:
                    hint = None
                ...
                for f in through._meta.private_fields:
                    ...
                    errors.append(
                        checks.Error(
                            f"The intermediary model '{qualified_model_name}' "
                            f"has no generic foreign key named '{src_field_name}'.",
                            hint=hint,
                            obj=self,
                            id="gm2m.E109",
                        )
                ...
                if field:
                    if not isinstance(field, ct.GenericForeignKey):
                        errors.append(
                            checks.Error(
                                f"'{through._meta.object_name}.{src_field_name}' is not a generic foreign key.",
                                hint=hint,
                                obj=self,
                                id="gm2m.E110",
                            )
    ...
    def contribute_to_class(self, cls, virtual_only=False):
        ...
        if not self.through:
            self.set_init("through", create_gm2m_intermediary_model(self.field, cls))
        ...
        def calc_field_names(rel):
            ...
            if is_fake_model(rel.through):
                ...
                # actual model
                for f, k in zip(rel.through_fields, ("src", "tgt", "tgt_ct", "tgt_fk")):
                    tf_dict[k] = f
                rel.through._meta._field_names = tf_dict
                return

            if rel.through_fields:
                tf_dict["src"], tf_dict["tgt"] = rel.through_fields[:2]
                for gfk in rel.through._meta.private_fields:
                    if gfk.name == tf_dict["tgt"]:
                        break
                else:
                    raise FieldDoesNotExist(
                        f'Generic foreign key "{tf_dict["tgt"]}" does not '
                        f'exist in through model "{rel.through._meta.model_name}"'
                    )
                ...
                for f in rel.through._meta.fields:
                    try:
                        ...
                        continue
                    if remote_field and (
                        remote_field.model == rel.field.model
                        or remote_field.model
                        == f"{rel.field.model._meta.app_label}.{rel.field.model._meta.object_name}"
                    ):
                        tf_dict["src"] = f.name
                        break
        ...
        if isinstance(self.through, str):
            ...
            lazy_related_operation(resolve_through_model, cls, self.through, r=self)
        else:
    ...
    def get_accessor_name(self, model=None):
        return self.related_name or (self.field.model._meta.model_name + "_set")
