...
class Options(options.Options):
    def get_field(self, field_name):
        ...
        try:
            ...
            if (
                field_name
                == self._forward_fields_map[SRC_ATTNAME].related_model._meta.model_name
            ):
                return super().get_field(SRC_ATTNAME)
            raise e
...
def create_gm2m_intermediary_model(field, klass):
    ...
    db_table = db_backends_utils.truncate_name(
        f"{klass._meta.db_table}_{field.name}", connection.ops.max_name_length()
    )
    ...
    meta_kwargs = {
        ...
        "verbose_name": f"{model_name}-generic relationship",
        "verbose_name_plural": f"{model_name}-generic relationships",
